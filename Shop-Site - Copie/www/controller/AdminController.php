...
class AdminController extends Controller {
    ...
    public function display() {
        ...
        $action = htmlspecialchars($_GET['action'] . "Action");
    ...
    private function removeAction() {
        ...
        $adminRepository = new AdminRepository();
        $result = $adminRepository->removeOne(htmlspecialchars($_GET['id']));
        $text = "";
    ...
    private function insertAction() {
        ...
        //Get the old and now dateTime 
        $dateNow = new DateTime("now");
        $datePaste = array_values($_SESSION['CSRF'])[0];
        $intervalMinute = $dateNow->diff($datePaste)->i;

        $text = "Vous n'êtes pas connnecté ! ";

        //Check the CSRF
        if (key($_SESSION['CSRF']) === $CSRF && $intervalMinute <= 30 ) {
            unset($_SESSION['CSRF']);
    ...
    private function formUpdateAction(){
        ...
        $product = $shopRepository->findOne(htmlspecialchars($_GET['id']));
    ...
    public function listUsersAction(){
        htmlspecialchars($_SESSION['field']);

        $adminRepository = new AdminRepository();
        $users=$adminRepository->rawQuery("select " . $_SESSION['field'] . " FROM t_user",PDO::FETCH_NUM);
